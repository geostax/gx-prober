...
public class HttpClientUtil {
	...
	public static byte[] postRequest(String reqURL, byte[] date) throws Exception {
		...
		try {
			// init post
			/*
			 * if (params != null && !params.isEmpty()) { List<NameValuePair> formParams =
			 * new ArrayList<NameValuePair>(); for (Map.Entry<String, String> entry :
			 * params.entrySet()) { formParams.add(new BasicNameValuePair(entry.getKey(),
			 * entry.getValue())); } httpPost.setEntity(new UrlEncodedFormEntity(formParams,
			 * "UTF-8")); }
			 */
